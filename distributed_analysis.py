...
def calculate_ideal_chunk_size(origins_count, destinations_count):
    ...
    if origins_count > destinations_count:
        ...
        logging.info(
            f"Will split {origins_count} origins into {number_of_chunks} chunks of ~{ideal_chunk_size} each")
...
def run_analysis(testing_func, r5py_network, origins_gdf, destinations_gdf, departure_datetime,
                 transport_modes):
    """
...
if __name__ == "__main__":
    ...
    current_osm_pbf_path = os.path.join(Path(__file__).parent, "input_data", "pbf_data",
                                        "cmap_road_network.pbf")
    ...
    for departure_time in departure_times:
        ...
        transit_travel_times_path = os.path.join(output_data_dir_path, "travel_times", "current_transit")

        # make sure output directory exists
        os.makedirs(transit_travel_times_path, exist_ok=True)

        transit_output_csv_path = os.path.join(transit_travel_times_path, f"{analysis_name}.csv")
        travel_times_df.to_csv(transit_output_csv_path, index=False)
        logging.info(f"Saved travel times for {analysis_name} to {transit_output_csv_path}")

    # now analysis for driving
    logging.info("Starting driving analysis")
    driving_transport_modes = [r5py.TransportMode.CAR]

    # run the analysis
    for departure_time in departure_times:
        logging.info(f"Running driving analysis for departure time: {departure_time}")

        # analysis name to keep track of each run
        analysis_name = f"current_network_driving_{departure_time.strftime('%Y%m%d_%H%M')}"
        # run the analysis
        ...
        travel_times_df = run_analysis(testing_func=calculate_travel_times_chunk,
                                       ...
                                       destinations_gdf=taz_gdf,
                                       departure_datetime=departure_time,
                                       transport_modes=driving_transport_modes
                                       )

        # save results to csv
        output_data_dir_path = os.path.join(Path(__file__).parent, "output_data")
        car_travel_times_path = os.path.join(output_data_dir_path, "travel_times", "driving")

        # make sure output directory exists
        os.makedirs(car_travel_times_path, exist_ok=True)

        car_output_csv_path = os.path.join(car_travel_times_path, f"{analysis_name}.csv")
        travel_times_df.to_csv(car_output_csv_path, index=False)
        logging.info(f"Saved driving travel times for {analysis_name} to {car_output_csv_path}")
